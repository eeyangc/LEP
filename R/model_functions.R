...
generate_pvalue <- function(n, p, h, rho, coef0, u, v, geno){
     if(geno == 0){
       x <- createX(n, p, rho)
     }
     else{
       x <- createGeno(n, p, rho)
     }
     coef <- numeric(p)
     if(u > 0 && v > 0){
...
setClass("Draw", representation(y = "matrix", pvalue = "matrix", pvalue0 = "matrix", ty = "matrix", train_index = "numeric", test_index = "numeric"))
  ...
  new_model(name = model_name, label = sprintf("n = %s, p = %s, h = %s, s = %s, rho = %s,  n0 = %s, u=%s, v=%s, K = %s, geno =%s", n, p, h, s, rho, n0, u, v, K,geno),
            ...
            simulate = function(x, se, mu, N, nsim) {
              ...
              for(i in 1:nsim){
                PVal <- matrix(0, p, K)
                for(k in 1:K){
                  pvalue <- generate_pvalue(n0, p, h0, rho, (beta != 0), u, v,geno)
...
make_sparse_linear_model <- function(n, p, h, s, rho, u, n0, h0, K, geno, model_name = "slm") {
  ...
  new_model(name = model_name, label = sprintf("n = %s, p = %s, h = %s, s = %s, rho = %s,  n0 = %s, u=%s, v=%s, K = %s, geno =%s", n, p, h, s, rho, n0, u, v, K,geno),
            params = list(x = x, beta = beta, mu = mu, se = se, n = n,
                          p = p, h = h, tx = tx, mut = mut, n0 = n0, geno = geno),
...
make_sparse_linear_model_dependent_pvalue <- function(n, p, h, s, rho, u, v, n0, h0, dr , K = 1, model_name = "slm_dependent") {
  x <- createX(n, p, rho)
  tx <- createX(n, p, rho)
  se <- 4
  ...
  new_model(name = model_name, label = sprintf("n = %s, p = %s, h = %s, s = %s, rho = %s,  n0 = %s, u=%s, v=%s, dr = %s", n, p, h, s, rho, n0, u, v, dr),
            params = list(x = x, beta = beta, mu = mu, se = se, n = n,
                          p = p, h = h, tx = tx, mut = mut, n0 = n0, G = G),
            simulate = function(x, mu, se,  mut, nsim, G = G) {
              ...
              for(i in 1:nsim){
                PVal <- matrix(0, p, K)
                for(k in 1:K){
                  pvalue <- LEPSimu::generate_pvalue(n0, p, h0, rho, (G[, k + 1] != 0), 1, 1, 1)
                  PVal[,k] <- as.matrix(pvalue)
                }
                y <- as.matrix(mu + sqrt(se) * rnorm(n))
                ty <- as.matrix(mut + sqrt(se) * rnorm(n))
                obj <- LEPSimu::SingleVariableScan(x, y)
                draw <- new("Draw", y = y, pvalue = PVal, pvalue0 = as.matrix(obj$pval), ty = ty,
